...
- **`services/cleanup.js`** — hourly scheduler; also runnable standalone (`npm run cleanup`, filesystem-only — no store is registered in that process). Also prunes finished download-job records (`sweepJobs`) and reconciles the `downloads` table (`completeStale` / `expireMissing` / `failStale`, in that order).
